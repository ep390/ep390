...
type ScoredItem = RubricItem & { score: number | null };
...
export function Grader({
  ...
  }, [rubricItems, scoresById]);

  const totalMax = useMemo(
    () => scoredItems.reduce((sum, it) => sum + it.maxPoints, 0),
    [scoredItems]
  );
  const totalScore = useMemo(
    () =>
      scoredItems.reduce((sum, it) => sum + (typeof it.score === "number" ? it.score : 0), 0),
    [scoredItems]
  );
